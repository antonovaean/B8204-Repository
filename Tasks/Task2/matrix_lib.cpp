//
// Created by GregorySysoev on 05.01.2018.
//
...
MatrixLib::Matrix::Matrix(MSIZE_T rows, MSIZE_T columns) throw (std::invalid_argument) { /*TODO(Achtung: ошибка)*/
...
Matrix minor(const Matrix &matrix, int l, int k) throw (std::logic_error) { /*TODO(Achtung: ошибка)*/
...
MITEM_T MatrixLib::Matrix::getDeterminant() const throw (std::logic_error) { /*TODO(Achtung: ошибка)*/
...
Matrix MatrixLib::Matrix::getInverse() const throw (std::logic_error) { /*TODO(Achtung: ошибка)*/
...
Matrix MatrixLib::operator*(const Matrix &a, const Matrix &b) throw (std::invalid_argument) /*TODO(Achtung: ошибка)*/
{
    if (a.columns != b.rows)
...
Matrix MatrixLib::operator+(const Matrix &a, const Matrix &b) throw (std::invalid_argument)
{
    if (a.rows != b.rows || a.columns != b.columns)
        throw std::invalid_argument/*("Invalid size")*/; /*TODO(Achtung: ошибка)*/
